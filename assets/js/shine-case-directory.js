    ...
    function openCaseDetail(row) {
      ...
      const queue = row["ISELL QUEUE"] || "";
      const concern = row["CASE RECORD TYPE"] || "";
      const action = row["Automatic Case Owner"] || "";
      const email = row["CORRECT CASE OWNER"] || "";
      const assignment = row["Additional Information Required When Moving"] || "";
    ...
    function initCaseDirectoryPage() {
      ...
      // --- Load Excel file ---
      fetch("assets/references/shine-case-directory.xlsx")
        ...
        .catch(err => {
          ...
          emptyState.innerHTML = `
            ...
            <div>Failed to load assets/references/shine-case-directory.xlsx</div>
          `;
          updateCounts([]);
        });
